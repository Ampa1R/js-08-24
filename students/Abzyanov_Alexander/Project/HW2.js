...
class cartItem {
    // Этот класс может быть создан путем наследования класса ProductItem, с добавлением количетва данного товара в корзине
    ...
    render() {
        return `<div class="goods-item" data-id="${this.id}">
                ...
                <div class="desc">
                    <h3>${this.title}</h3>
                    <p>${this.price} USD</p>
                    <button class="goodsAddButton">Купить</button>
                </div>
            </div>`
    }
}
...
class GoodsList {
    ...
    renderTotalCost () {
        ...
        block.insertAdjacentHTML('beforeend', `<br> Общая стоимость каталога: <br> ${this.getTotalCostOfProductList ()} USD.`);
